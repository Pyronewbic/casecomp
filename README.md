...
| `--raw-suffix` | Extra words for raw mode (default via config is `raw`) |
...
|------|------------------|-----------------|
| **raw** | `{card} {rawSearchSuffix}` (default suffix `raw`) | Drops titles that look like graded slabs (PSA/BGS/CGC-style). |
| **slab** | `{card} {provider} {grade}` | Keeps titles that plausibly mention that grader + grade. |

Examples:

```bash
# Raw (default suffix "raw" from CONFIG)
