...
TECHNIQUE: Describe what you observe rather than computing exact ratios. Compare left vs right borders, then top vs bottom borders. Back centering is often the grade limiter — many cards with good front centering fail on the back.
...
async function gradeSubgrade(subgrade, imageBlocks, config, extraBlocks = []) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  ...
  return { score: clampSub(parsed.ok.score), confidence: clampConf(parsed.ok.confidence), detail: parsed.ok.detail || "", _tokens: { input: usage.input_tokens || 0, output: usage.output_tokens || 0 } };
}
...
export async function gradeDetailedLLM(frontUrl, backUrl, config, extraImages = []) {
  ...
  const subgradeCalls = [
    gradeSubgrade("centering_front", [frontBlock], config),
    gradeSubgrade("centering_back", hasBack ? [backBlock] : [frontBlock], config),
  ...
  return {
    ...
    subgradeDetails,
    cardDetection: {
      front: frontDetect.cropped ? frontDetect.bounds : null,
      back: backDetect?.cropped ? backDetect.bounds : null,
    },
