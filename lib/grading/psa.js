...
// ── Main entry point ─────────────────────────────────────────────────
export async function getPsaGradingSignal(cardName, { log = console.log, _refresh = false, _cachePrefix = "" } = {}) {
  const key = `${_cachePrefix}${cardName.toLowerCase().trim().replace(/[/\\. ]+/g, "_")}`;

  if (!_refresh) {
    const { value: cached, stale } = await cacheGetStale(POP_COLLECTION, key);
    if (cached) {
      log(`  psa pop: cache hit for "${cardName}"${stale ? " (stale, refreshing)" : ""}`);
      if (stale && !_cachePrefix) {
        getPsaGradingSignal(cardName, { log: () => {}, _refresh: true }).catch(() => {});
      }
      return cached;
    }
  }
  ...
  if (specId && typeof specId === "object" && specId.notFound) {
    const age = Date.now() - new Date(specId.checkedAt).getTime();
    if (age < NOT_FOUND_TTL_MS) {
      log(`  psa pop: spec not-found cache hit (checked ${Math.round(age / 3_600_000)}h ago)`);
      return null;
    }
    specId = null;
  }
  if (!specId) {
    ...
    if (specId) {
      await cacheSet(SPEC_COLLECTION, key, specId, 0);
    } else {
      await cacheSet(SPEC_COLLECTION, key, { notFound: true, checkedAt: new Date().toISOString() }, NOT_FOUND_TTL_MS);
    }
  } else {
