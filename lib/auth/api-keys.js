...
export async function createApiKey({ label, rateLimit = 60, ownerId = null }) {
  const fs = getDb();
...
export async function listAllKeys() {
  ...
  return snap.docs.map(d => {
    const data = d.data();
    return { id: data.id, keyPrefix: data.keyPrefix, label: data.label, ownerId: data.ownerId, rateLimit: data.rateLimit, active: data.active, createdAt: data.createdAt, lastUsedAt: data.lastUsedAt, requestCount: data.requestCount };
  });
}

export async function listKeysByOwner(ownerId) {
  const fs = getDb();
  if (!fs) return [];
  const snap = await fs.collection(COLLECTION).where("ownerId", "==", ownerId).orderBy("createdAt", "desc").get();
  return snap.docs.map(d => {
    const data = d.data();
    return { id: data.id, keyPrefix: data.keyPrefix, label: data.label, rateLimit: data.rateLimit, active: data.active, createdAt: data.createdAt, lastUsedAt: data.lastUsedAt, requestCount: data.requestCount };
  });
}
