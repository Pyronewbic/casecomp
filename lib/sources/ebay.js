...
import { cachePath } from "../data/cache.js";
import { cacheGet, cacheGetStale, cacheSet } from "../data/firestore.js";
import {
  filterByLanguage,
...
} from "../search/ebayCategories.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const USAGE_FILE = "ebay-usage.json";
const ACTIVE_CACHE_COL = "cache-ebay-active";
const SOLD_CACHE_COL = "cache-ebay-sold";
const INSIGHTS_CACHE_COL = "cache-ebay-insights";
...
function docKey(s) { return s.replace(/[/\\. ]+/g, "_").substring(0, 1500); }
...
async function markMarketplaceInsightsForbidden403(summary) {
  await cacheSet(INSIGHTS_CACHE_COL, "forbidden403", { http403: true, detail: summary }, INSIGHTS_FORBIDDEN_TTL_MS);
}

async function marketplaceInsightsCooldownActive() {
  const j = await cacheGet(INSIGHTS_CACHE_COL, "forbidden403");
...
function upgradeEbayImageRes(url) {
...
function normalizeBrowseItem(item) {
  ...
  const add = (item.additionalImages || []).map((x) => upgradeEbayImageRes(x.imageUrl)).filter(Boolean);
  ...
  return {
    ...
    listingGradeLabel,
    imageUrl: upgradeEbayImageRes(item.image?.imageUrl) || null,
    additionalImages: add,
    seller: item.seller ? {
      feedbackScore: item.seller.feedbackScore ?? null,
      feedbackPercentage: item.seller.feedbackPercentage ? parseFloat(item.seller.feedbackPercentage) : null,
...
function insightsToSold(entry) {
  ...
  return {
    ...
    currency:
      ...
      "USD",
    endedDate: date,
    imageUrl: upgradeEbayImageRes(entry.image?.imageUrl) || null,
    raw: entry,
  };
}
...
function parseSoldTilesFromHtml(html) {
  ...
  function pushRow({ title, link, priceText, ended, img }) {
    ...
    items.push({
      ...
      imageUrl: upgradeEbayImageRes(img) || null,
      raw: { priceText },
    });
  }
  ...
  $(".s-item").each((i, el) => {
    if (i === 0) return;
    const $el = $(el);
    // Skip rows with no completed-listing marker — eBay injects active/sponsored
    // rows into the LH_Sold=1&LH_Complete=1 results page that have no ended date.
    if (!$el.find(".s-item__ended-date, .POSITIVE").length) return;
  ...
  if (!items.length) {
    $(".s-card").each((_, el) => {
      const $el = $(el);
      if (!$el.find(".s-item__ended-date, .POSITIVE, .s-card__caption").length) return;
      const link =
...
export async function searchActive(
  ...
  const cp = config._cachePrefix || "";
  const key = `${cp}${query}::vship::${destKey}::${langKey}`;
  const dk = docKey(key);
  if (!refresh && !noEbay) {
  ...
  if (noEbay) {
    const ent = await cacheGet(ACTIVE_CACHE_COL, dk);
  ...
  let shipLookups = 0;
  const skipShipCheck = !process.env.EBAY_SHIP_LOOKUP_ENABLED;
  if (!skipShipCheck) {
    const result = await enrichRowsShipToDestinations(
      payload.candidatePool,
      destinations,
      { getToken, on401 },
    );
    shipLookups = result.lookups;
  }

  const itemsByCountry = {};
  if (skipShipCheck) {
    const worldwide = payload.candidatePool.slice(0, config.resultsPerCard);
    for (const iso of destinations) {
      itemsByCountry[iso] = worldwide;
    }
  } else {
    for (const iso of destinations) {
  ...
  await cacheSet(ACTIVE_CACHE_COL, docKey(key), outPayload, ACTIVE_TTL_MS);
...
export async function searchSold(
  ...
  const cp = config._cachePrefix || "";
  const key = `${cp}${query}::${listingLanguagesCacheTag(langs)}::${soldBrowser ? "pw" : "http"}${cacheKeyTag}`;
  const soldDk = docKey(key);
  if (!refresh && !noEbay) {
  ...
  if (noEbay) {
    const cached = await cacheGet(SOLD_CACHE_COL, soldDk);
    return {
      items: cached?.items || [],
      source: cached?.items?.length ? "cache" : "no-ebay",
      pipeline: {
        fetched: cached?.items?.length || 0,
        afterLanguage: cached?.items?.length || 0,
        afterRelevance: cached?.items?.length || 0,
        afterListingFormat: cached?.items?.length || 0,
      },
    };
  }
  ...
  await cacheSet(SOLD_CACHE_COL, docKey(key), {
    items: top,
    source,
    pipeline: {
      ...
      soldBrowseGetItemCalls: langs.length > 0 ? soldBrowseGetItemCalls : 0,
