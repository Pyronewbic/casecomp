...
const JP_SET_CODES = ["sv8a", "sv8", "sv7", "sv6", "sv5k", "sv5m", "sv5a", "sv4a", "sv4", "sv3a", "sv3", "sv2a", "sv2d", "sv2p", "sv1s", "sv1v", "m4", "m2a", "m1", "s12a", "s12", "s11a", "s11", "s10", "s9a", "s9", "s8a", "s8b"];

export function querySeeksJapaneseMarket(query) {
  const q = (query || "").toLowerCase();
  if (/\bjapanese\b/.test(q) || /\bjapan\b/.test(q) || /\bjpn\b/.test(q)) return true;
  if (JP_REGEX.test(query)) return true;
  for (const code of JP_SET_CODES) {
    if (q.includes(code)) return true;
  }
  const denomMatch = q.match(/\/0*(\d+)/);
  if (denomMatch) {
    const denom = denomMatch[1];
    const JP_DENOMS = ["187", "83", "193", "78", "81", "101", "108", "131", "190", "172", "165", "162", "100"];
    if (JP_DENOMS.includes(denom)) return true;
  }
  return false;
}
...
export function filterRelevantResults(results, query) {
  ...
  if (querySeeksJapaneseMarket(query)) {
    ...
    out = out.filter((r) => {
      if (titleLooksChineseRegionalListing(r.title)) return false;
      const tl = (r.title || "").toLowerCase();
      if (tl.includes("korean") || tl.includes("(korean)")) return false;
      return true;
    });
  ...
  const beforeSeller = out.length;
  out = out.filter(i => {
    if (!i.seller) return true;
    if (i.seller.feedbackScore !== null && i.seller.feedbackScore < 5) return false;
    if (i.seller.feedbackPercentage !== null && i.seller.feedbackPercentage < 90) return false;
    return true;
  });
  if (beforeSeller > out.length) stats.droppedLowFeedback = beforeSeller - out.length;

  if (out.length >= 3) {
    const prices = out.map(i => i.totalCost || i.price).filter(Boolean).sort((a, b) => a - b);
    if (prices.length >= 3) {
      const median = prices[Math.floor(prices.length / 2)];
      const suspiciousFloor = median * 0.2;
      const before = out.length;
      out = out.filter(i => {
        const p = i.totalCost || i.price;
        return !p || p >= suspiciousFloor;
      });
      stats.afterPriceFloor = out.length;
      if (before > out.length) stats.droppedSuspiciousPrice = before - out.length;
    }
  }

  console.log(
    `[filterRelevantResults] query="${query}" in=${stats.input} ≥50%kw=${stats.afterKeywordRatio}${stats.keywordFallback ? " (name+blocklist fallback)" : ""} blocklist=${stats.afterBlocklist} pokemon=${stats.afterPokemonName}${stats.droppedChineseRegional ? ` −CN=${stats.droppedChineseRegional}` : ""}${stats.droppedLowFeedback ? ` −seller=${stats.droppedLowFeedback}` : ""}${stats.droppedSuspiciousPrice ? ` −scam=${stats.droppedSuspiciousPrice}` : ""}`,
  );
...
export function isGradedCard(query) {
  return /\b(PSA|BGS|CGC|TAG|ACE)\s+\d/i.test(query);
}
