...
class FileCache {
  ...
  async load() {
    if (!this.data) {
      try { this.data = JSON.parse(await fs.readFile(this.path, "utf8")); }
      catch { this.data = {}; }
    }
    return this.data;
  }
  async save() {
    try { await fs.writeFile(this.path, JSON.stringify(this.data, null, 2), "utf8"); } catch {}
  }
}

const popCache  = new FileCache("psa-pop-cache.json");
const specCache = new FileCache("psa-spec-cache.json");
...
export async function getPsaGradingSignal(cardName, { log = console.log } = {}) {
  ...
  const cache = await popCache.load();
  const specs = await specCache.load();
