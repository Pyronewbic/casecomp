...
let cardIndex = [];
const tcgdexSetNames = new Map();

async function fetchSetNames(timeout = 15000) {
  const map = new Map();
  const fetches = ["en", "ja"].map(async (lang) => {
    try {
      ...
      for (const s of sets) {
        ...
        if (id && s.name && !map.has(id)) map.set(id, s.name);
      }
    } catch {}
  });
...
async function fetchAndBuildIndex() {
  const [enCards, jaCards, enRarities, jaRarities, setNames] = await Promise.all([
    fetchCards("en").catch(() => []),
    fetchCards("ja").catch(() => []),
    fetchRarities("en").catch(() => new Map()),
    fetchRarities("ja").catch(() => new Map()),
    fetchSetNames().catch(() => new Map()),
  ]);
  if (!enCards.length && !jaCards.length) return null;
  const rarityMap = new Map([...enRarities, ...jaRarities]);
  for (const [id, name] of setNames) tcgdexSetNames.set(id, name);
  return buildIndexFromApi(enCards, jaCards, rarityMap);
}

export async function initCardDatabase() {
  const setNamesPromise = fetchSetNames().then(names => {
    for (const [id, name] of names) tcgdexSetNames.set(id, name);
    if (names.size) console.log(`Card database: ${names.size} set names from TCGdex`);
  }).catch(() => {});
...
function resolveSetName(code) {
  return SET_NAME_MAP[code] || tcgdexSetNames.get(code) || code;
}
