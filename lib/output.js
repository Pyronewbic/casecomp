...
import { PSA_TIERS } from "./psaTiers.js";
...
export function gradingDecisionLines(rawBlock, psa9Block, psa10Block) {
  const rawSoldPrices = (rawBlock?.sold || []).map(s => Number(s.price)).filter(v => v > 0);
  const rawMedian = mean(rawSoldPrices);

  const psa9Prices  = (psa9Block?.sold  || []).map(s => Number(s.price)).filter(v => v > 0);
  const psa10Prices = (psa10Block?.sold || []).map(s => Number(s.price)).filter(v => v > 0);
  const psa9Median  = mean(psa9Prices);
  const psa10Median = mean(psa10Prices);

  // Warn when sold comp count is thin (< 3)
  const fmtMedianWithWarn = (m, count) => {
    if (m == null) return "—";
    const warn = count < 3 ? ` ⚠ ${count} comp${count !== 1 ? "s" : ""}` : "";
    return `${money(m)}${warn}`;
  };

  // EV — needs PSA pop signal on the raw block
  const sig = rawBlock?.psaSignal;
  ...
  const hasEV = p10 != null && psa10Median != null && rawMedian != null;

  const lines = [];
  lines.push("**Grading Decision** (PSA submission analysis)");
  lines.push(
    `Raw sold avg: ${fmtMedianWithWarn(rawMedian, rawSoldPrices.length)} | ` +
    `PSA 9 sold avg: ${fmtMedianWithWarn(psa9Median, psa9Prices.length)} | ` +
    `PSA 10 sold avg: ${fmtMedianWithWarn(psa10Median, psa10Prices.length)}`
  );
  lines.push("");

  if (hasEV) {
    lines.push("| Tier | Fee | Net PSA 9 | Upside | Net PSA 10 | Upside | EV | vs Raw |");
    lines.push("|------|----:|----------:|-------:|-----------:|-------:|---:|-------:|");
  } else {
    lines.push("| Tier | Fee | Net PSA 9 | Upside | Net PSA 10 | Upside |");
    lines.push("|------|----:|----------:|-------:|-----------:|-------:|");
  }

  const fmtUp = (v) => v == null ? "—" : `${v >= 0 ? "+" : ""}${v.toFixed(0)}%`;
  for (const { name, fee } of PSA_TIERS) {
    const net9  = psa9Median  != null ? Math.round(psa9Median  - fee) : null;
    const net10 = psa10Median != null ? Math.round(psa10Median - fee) : null;
    ...
    if (hasEV) {
      // 3-outcome: p10×net10 + p9×net9 + pOther×rawMedian; 2-outcome fallback when no ratio
      let ev;
      if (p9 != null && psa9Median != null) {
        ev = p10 * (psa10Median - fee) + p9 * (psa9Median - fee) + pOther * rawMedian;
      } else {
        ev = p10 * (psa10Median - fee) + (1 - p10) * rawMedian;
      }
      ev = Math.round(ev);
  ...
  if (hasEV) {
    ...
    lines.push(`_Net = sold avg − submission fee. EV = probability-weighted return (${evNote}). Upside vs raw sold avg._`);
  } else {
    const noEvReason = !sig ? "PSA pop unavailable — resets daily at 100 req/day" : "PSA 10 rate unknown";
    lines.push(`_Net = sold avg − submission fee. Upside vs raw sold avg. EV not shown (${noEvReason})._`);
  }
  return lines;
}
...
export async function writeMarkdown(results, config, meta) {
  ...
  for (const block of results) {
    ...
    if (block.gradeDecision) {
      const decLines = gradingDecisionLines(block, block.gradeDecision.psa9, block.gradeDecision.psa10);
