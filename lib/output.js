...
export function gradingDecisionLines(rawBlock, psa9Block, psa10Block) {
  const rawActive = Object.values(rawBlock?.activeByCountry || {}).flat();
  const seen = new Set();
  const rawPrices = rawActive
    .filter(r => { const k = r.itemId || r.itemWebUrl; return k && !seen.has(k) && seen.add(k); })
    .map(r => r.totalCost).filter(v => v != null && v > 0);
  ...
  const psa9Median  = median(psa9Prices);
  const psa10Median = median(psa10Prices);

  const lines = [];
  lines.push("**Grading Decision** (PSA submission analysis)");
  lines.push(
    `Raw ask median: ${rawMedian != null ? money(rawMedian) : "—"} | ` +
    `PSA 9 sold median: ${psa9Median != null ? money(psa9Median) : "—"} | ` +
    `PSA 10 sold median: ${psa10Median != null ? money(psa10Median) : "—"}`
  );
  lines.push("");
  ...
  lines.push("");
  lines.push("_Net = median sold − submission fee. Upside vs raw median ask._");
  return lines;
}
